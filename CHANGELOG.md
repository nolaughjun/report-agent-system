...
### 修复
- **数据收集质量重试**: 质量不足时返回规划阶段重新生成检索词，而非直接进入撰写
  - 新增 `gather_retry_count` 和 `max_gather_retry` 状态字段
  - 规划节点在重试时使用改进版提示词，包含之前的问题和建议
- **质量审核流程**: 质量不达标时进入人工审核等待用户决策，而非自动改进循环
- **用户修改意见隔离**: 明确告知 LLM 用户修改意见仅供参考，不应出现在报告中

### 改进
- 完善测试覆盖（新增 9 个数据收集重试测试用例）
- 优化路由逻辑，用户主动修改不受 max_retry 限制
